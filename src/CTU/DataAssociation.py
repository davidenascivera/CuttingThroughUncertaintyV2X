...
class DataAssociationManager:
    ...
    def solve(self, verbose: bool = False) -> np.ndarray:
        '''
        Function to solve the assignment problem using the Hungarian algorithm. if verbose is True it prints the valid assignments.
        ...
        try:
            ...
            logger.error("Assignment failed: cost matrix is infeasible")
            logger.debug(f"Cost matrix at failure:\n{self.cost_matrix}")
            logger.debug(f"Resetting the frozenset blacklist .........................")
            self.blacklist = set()
            self.compute_cost_matrix(verbose=False)
            try:
                row_ind, col_ind = linear_sum_assignment(self.cost_matrix)
            except ValueError as e:
                logger.error("Assignment failed again: cost matrix is still infeasible")
                logger.debug(f"Cost matrix at failure:\n{self.cost_matrix}")
                return []
        ...
        for row_1, col_2 in zip(row_ind, col_ind):
            if row_1 < self.n_1 and col_2 < self.n_2:
                id_1 = track_sens_1_keys[row_1]
                id_2 = track_sens_2_keys[col_2]
                valid_assignments.append((id_1, id_2))
                
        # if verbose:
        #     logger.info(f"Valid assignments: {valid_assignments}")
        self.valid_assignments = valid_assignments
